...
import { ArweaveBlock } from './models/arweaveBlock';
import { ArweaveTransaction } from './models/arweaveTransaction';
...
export class ArweaveDB {
  ...
  private blocksDB = new Datastore({
    filename: getPath('storage/arweave-blocks.db'),
    autoload: true,
  });
  private txsDB = new Datastore({
    filename: getPath('storage/arweave-txs.db'),
    autoload: true,
  });
  ...
  constructor() {
    this.blocksDB.ensureIndex({ fieldName: 'indep_hash', unique: true }, err => {
      if (err) console.log('ensureIndexError', err);
    });
    this.blocksDB.ensureIndex({ fieldName: 'height', unique: true }, err => {
      if (err) console.log('ensureIndexError', err);
    });

    this.txsDB.ensureIndex({ fieldName: 'id', unique: true }, err => {
      if (err) console.log('ensureIndexError', err);
    });
  ...
  on(evt: 'transactions-added' | 'info-updated' | 'block-added', cb: Function) {
  ...
  private trigger(evt: 'transactions-added' | 'info-updated' | 'block-added', params: any = null) {
  ...
  async getBlock(blockHeight: number): Promise<ArweaveBlock> {
    return new Promise((resolve, reject) => {
      this.blocksDB.findOne({ _id: blockHeight }, (err, doc: ArweaveBlock) => {
        if (err) {
          console.log(err);
          return reject(err);
        }
  ...
  async getTransaction(txid: string): Promise<ArweaveTransaction> {
    return new Promise((resolve, reject) => {
      this.txsDB.findOne({ id: txid }, (err, doc: ArweaveTransaction) => {
        if (err) {
          console.log(err);
          return reject(err);
        }
  ...
  /*** Setters ***/
  private async addBlock(block: ArweaveBlock): Promise<ArweaveBlock> {
    return new Promise(resolve => {
      this.blocksDB.insert(block, (err: any, doc) => {
        if (err) {
          if (err.errorType !== 'uniqueViolated') console.log(err);
          return resolve(null);
        }

        this.trigger('block-added', doc);
        resolve(doc);
      });
    });
  }

  private async addTransaction(tx: ArweaveTransaction): Promise<ArweaveTransaction> {
    return new Promise(resolve => {
      this.txsDB.insert(tx, (err: any, doc) => {
        if (err) {
          if (err.errorType !== 'uniqueViolated') console.log(err);
          return resolve(null);
        }

        this.trigger('transactions-added', [doc]);
        resolve(doc);
      });
    });
  }

  private async addTransactions(txs: ArweaveTransaction[]): Promise<ArweaveTransaction[]> {
    return new Promise(async resolve => {
      this.txsDB.insert(txs, (err: any, docs) => {
        if (err) {
          if (err.errorType !== 'uniqueViolated') console.log(err);
          return resolve(null);
        }
  ...
  private async sync() {
    ...
    const go = async (): Promise<any> => {
      return new Promise(async (resolve) => {
        ...
        if (block.txs.length) {
          const txs = await Promise.all(block.txs.map(async (txId) => await this.getPeerTx(txId)));
  ...
  private async getPeerBlock(blockHeight: number): Promise<ArweaveBlock> {
    return new Promise(async (resolve) => {
  ...
  private async getPeerTx(txId: string): Promise<ArweaveTransaction> {
    return new Promise(async (resolve) => {
      const randIndex = Math.floor(Math.random() * this.activePeers.length);
